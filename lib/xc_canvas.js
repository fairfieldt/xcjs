...
_xcSpriteDraw = function(node) {
  ...
  return context.drawImage(node.sprite, 0, 0, node.width(), node.height(), 0 - (node.width() * node.anchorX()), 0 - (node.height() * node.anchorY()), node.width() * node.scaleX(), node.height() * node.scaleY());
};
...
xc_init = function() {
  ...
  clear = function() {
    return context.clearRect(0, 0, xc.canvasWidth, xc.canvasHeight);
  };
  fps = 60;
